...
* [checkpointer](https://github.com/Reddan/checkpointer). Decorator-based persistent memoization with automatic code-aware cache invalidation. The core architectural difference is how code dependencies are discovered: checkpointer uses **static inspection** (analyzing the function's global scope, type annotations, and object constructions at decoration time) whereas memodisk uses **runtime monitoring** (`sys.monitoring` callbacks during actual execution). This has practical consequences:
  - **Static inspection** (checkpointer) finds dependencies without executing the function, so the dependency set is available immediately and does not vary across calls. However, it may miss dependencies reached through dynamic dispatch, `getattr`, registry lookups, or conditional imports, and may include dependencies from code paths not actually taken for a given set of arguments.
  - **Runtime monitoring** (memodisk) captures the exact set of functions and files used for each specific call with specific arguments, so it never includes false positives from untaken branches. However, it requires executing the function at least once to establish the dependency set, and the first call incurs tracing overhead.

  Other differences are more incremental: checkpointer supports async functions, in-memory and custom storage backends, TTL-based expiry, captured global variables (`CaptureMe`/`CaptureMeOnce`), and fine-grained argument hashing via `HashBy`/`NoHash` type annotations. memodisk additionally tracks data file dependencies (files opened for reading), external process invocations, ambient time API usage, numpy random generator state, and installed package versions. checkpointer does not detect data dependency changes.
