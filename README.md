...
* [checkpointer](https://github.com/Reddan/checkpointer). Decorator-based persistent memoization with automatic code-aware cache invalidation. Compared to memodisk: checkpointer detects code changes via AST/source hashing (ignoring whitespace and comments) whereas memodisk uses bytecode hashing; checkpointer recursively hashes user-defined dependencies found through global-scope inspection and type annotations whereas memodisk discovers dependencies at runtime via `sys.monitoring`; checkpointer supports async functions, in-memory storage, custom storage backends, TTL-based expiry, captured global variables (`CaptureMe`/`CaptureMeOnce`), and fine-grained argument hashing via `HashBy`/`NoHash` type annotations; memodisk additionally tracks data file dependencies (files opened for reading), external process invocations, ambient time API usage, numpy random generator state, and installed package versions — none of which checkpointer handles. checkpointer does not detect data dependency changes.
